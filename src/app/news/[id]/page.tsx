...
export default function NewsDetailPage() {
  ...
  const handleShare = async () => {
    ...
    const title = news?.title || 'TIMS News';
    if (navigator.share) {
      try {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#F0E6CE', paddingBottom: '100px' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '60px 20px' }}>
        ...
        <div style={{ marginTop: '80px', padding: '40px', background: '#F0E6CE', borderRadius: '24px', border: '1px solid rgba(28, 13, 7, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '20px' }}>
          <div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            <button
              onClick={handleShare}
              style={{ width: '45px', height: '45px', borderRadius: '12px', border: 'none', background: '#00122e', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'background 0.2s' }}
              title={copied ? 'Link copied!' : 'Share article'}
            >
              {copied ? '✓' : <Share2 size={20} />}
            </button>
          </div>
        </div>
